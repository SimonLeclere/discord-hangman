...
class hangman {
	...
	async showProgress() {
		...
		if (this.message) await this.message.edit({ embed: embed });
		else this.message = await this.channel.send({ embed: embed });
	}
	...
	async start() {
		...
		const collector = this.channel.createMessageCollector(filter, { time: 900000 });
